...
def evaluate_ansatz_performance(qc: QuantumCircuit, target_state: Statevector, seed: int):
    ...
    return {
        "performance_state_fidelity": fidelity
    }
...
def run_experiments(
    ...
    for n in num_qubits_list:
        ...
        target_states = {
            "ghz": ghz_state(n),
            "ising": ising_evolved_state(n, t=0.5)
        }
            ...
            for reps in reps_list:
                for idx in range(num_instances):
                    for ansatz_type in ["su2", "linear_entanglement", "two_local"]:
                        ...
                        performance_metrics = evaluate_ansatz_performance(qc, target_state, seed=instance_seed)
    ...
    if "performance_state_fidelity" in df.columns:
        cols = [c for c in df.columns if c != "performance_state_fidelity"] + ["performance_state_fidelity"]
        df = df[cols]
